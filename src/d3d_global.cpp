...
#include "d3d_state.h"
#include "d3d_utils.h"
#include "d3d_immediate.h"
#include "d3d_array.h"
#include "d3d_object.h"
...
void D3DGlobal_Reset( void )
{
	...
	if (D3DGlobal.pIMBuffer) {
		...
		D3DGlobal.pIMBuffer = NULL;
	}
	if (D3DGlobal.pVABuffer) {
		delete D3DGlobal.pVABuffer;
		D3DGlobal.pVABuffer = NULL;
	}
...
void D3DGlobal_Cleanup( bool cleanupAll )
{
	...
	if (D3DGlobal.compiledVertexArray.compiledNormalData) {
		...
		D3DGlobal.compiledVertexArray.compiledNormalData = NULL;
	...
	if (D3DGlobal.compiledVertexArray.compiledColorData) {
		...
		D3DGlobal.compiledVertexArray.compiledColorData = NULL;
	...
	for (int i = 0; i < MAX_D3D_TMU; i++) {
		if (D3DGlobal.compiledVertexArray.compiledTexCoordData[i]) {
			...
			D3DGlobal.compiledVertexArray.compiledTexCoordData[i] = NULL;
	...
	if (D3DGlobal.pSwapChain) {
		...
		D3DGlobal.pSwapChain = NULL;
	}
	if (D3DGlobal.pDevice) {
		D3DGlobal.pDevice->Release();
		D3DGlobal.pDevice = NULL;
	}
...
==================
*/
DWORD D3DGlobal_GetRegistryValue( const char *key, const char *section )
{
	DWORD dwValue;
    HKEY hKey;
    LONG returnStatus;
	...
	if (returnStatus == ERROR_SUCCESS) {
		returnStatus = RegQueryValueEx(hKey, key, NULL, &dwType, (LPBYTE)&dwValue, &dwSize);
		if (returnStatus == ERROR_SUCCESS) {
			RegCloseKey(hKey);
			return dwValue;
		} else {
...
static D3DFORMAT D3DGlobal_GetDepthFormat(D3DFORMAT AdapterFormat)
{
	...
	int i;
	D3DFORMAT d3d_DepthFormats[] = {D3DFMT_D24S8, D3DFMT_D24X4S4, D3DFMT_D32, D3DFMT_D24X8, D3DFMT_D16, D3DFMT_UNKNOWN};
...
static D3DFORMAT D3DGlobal_GetAlphaFormat(D3DFORMAT AdapterFormat)
{
	...
	// first try to create formats with depthstencil buffer, then depth-only
	int i;
	D3DFORMAT d3d_ColorFormats[] = {D3DFMT_A8R8G8B8, D3DFMT_A4R4G4B4, D3DFMT_UNKNOWN};
...
static D3DFORMAT D3DGlobal_GetAdapterModeFormat( int width, int height, int bpp )
{
	...
	for (i = 0; ; i++)
	{
		...
		modecount = D3DGlobal.pD3D->GetAdapterModeCount(D3DADAPTER_DEFAULT, d3d_Formats[i]);
...
OPENGL_API HGLRC WINAPI wrap_wglCreateContext( HDC hdc )
{
	...
	if (!(D3DGlobal.hWnd = WindowFromDC(hdc))) {
	...
	// initialize direct3d
	if (!D3DGlobal.pD3D) {
		if ( NULL == (D3DGlobal.hD3DDll = LoadLibrary("d3d9.dll")) ) {
		...
		if ( NULL == (D3DGlobal.pD3D = d3dCreateFn(D3D_SDK_VERSION)) ) {
			logPrintf("wglCreateContext: failed to initialize Direct3D\n");
			return 0;
		}
		if (NULL == D3DGlobal.pD3D) {
	...
	D3DGlobal.settings.multisample = D3DGlobal_GetRegistryValue( "MultiSample", "Settings" );
	D3DGlobal.settings.projectionFix = D3DGlobal_GetRegistryValue( "ProjectionFix", "Settings" );
	D3DGlobal.settings.texcoordFix = D3DGlobal_GetRegistryValue( "TexCoordFix", "Settings" );
	D3DGlobal.settings.useSSE = D3DGlobal_GetRegistryValue( "UseSSE", "Settings" );
	...
	// get window style
	HWND hwndParent;
	if ((hwndParent = GetParent(D3DGlobal.hWnd)) != NULL) {
	...
	//first clear
	D3DGlobal.pDevice->Clear( 0, NULL, (D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER|D3DCLEAR_STENCIL) & ~D3DGlobal.ignoreClearMask, 
...
OPENGL_API HGLRC WINAPI wrap_wglGetCurrentContext( void )
{
	return D3DGlobal.hGLRC;
}

OPENGL_API HDC WINAPI wrap_wglGetCurrentDC( void )
{
...
OPENGL_API BOOL WINAPI wrap_wglSwapBuffers(HDC hDC)
{
	if (!D3DGlobal.hGLRC)
		return FALSE;
	...
	if (D3DGlobal.sceneBegan)
	{
		...
		if (D3DGlobal.vSync)
			hr = D3DGlobal.pDevice->Present( NULL, NULL, NULL, NULL );
		else
			hr = D3DGlobal.pSwapChain->Present( NULL, NULL, NULL, NULL, D3DPRESENT_DONOTWAIT );
...
OPENGL_API BOOL WINAPI wrap_wglSetPixelFormat(HDC hdc, int p, CONST PIXELFORMATDESCRIPTOR *pfd) 
{ 
	// just silently pass the PFD through unmodified
	return TRUE;
}

OPENGL_API BOOL WINAPI wrap_wglCopyContext( HGLRC hglrcSrc, HGLRC hglrcDst, UINT mask )
{
	return FALSE;
}

OPENGL_API HGLRC WINAPI wrap_wglCreateLayerContext( HDC hdc, int iLayerPlane )
{
	return (HGLRC)0;
}

OPENGL_API BOOL WINAPI wrap_wglDescribeLayerPlane( HDC hdc, int iPixelFormat, int iLayerPlane, UINT nBytes, LPLAYERPLANEDESCRIPTOR plpd )
{
	return FALSE;
}

OPENGL_API int WINAPI wrap_wglGetLayerPaletteEntries( HDC hdc, int iLayerPlane, int iStart, int cEntries, COLORREF *pcr )
{
	return 0;
}
...
OPENGL_API int WINAPI wglGetSwapInterval(void)
{
