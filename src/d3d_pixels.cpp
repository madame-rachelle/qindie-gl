...
inline void D3DPixels_ModifyBGR( GLubyte *bgr )
{
	...
	bgr[2] = QINDIEGL_CLAMP( fR * 255.0f );
	bgr[1] = QINDIEGL_CLAMP( fG * 255.0f );
	bgr[0] = QINDIEGL_CLAMP( fB * 255.0f );
}
...
inline void D3DPixels_ModifyBGRA( GLubyte *bgra )
{
	...
	bgra[2] = QINDIEGL_CLAMP( fR * 255.0f );
	bgra[1] = QINDIEGL_CLAMP( fG * 255.0f );
	bgra[0] = QINDIEGL_CLAMP( fB * 255.0f );
	bgra[3] = QINDIEGL_CLAMP( fA * 255.0f );
}
...
inline void D3DPixels_ModifyABGR( GLubyte *bgra )
{
	...
	bgra[3] = QINDIEGL_CLAMP( fR * 255.0f );
	bgra[2] = QINDIEGL_CLAMP( fG * 255.0f );
	bgra[1] = QINDIEGL_CLAMP( fB * 255.0f );
	bgra[0] = QINDIEGL_CLAMP( fA * 255.0f );
}
				...
				if (pack_mode == PP_TYPE_UNPACKED) {
					for ( int l = 0; l < fmtpixelsize; l++ ) {
						...
						if (D3DState.ClientPixelStoreState.unpackSwapBytes) {
							if (sizeof(T) == 4)
								pixeldata = (T)D3D_ByteSwap32(pixeldata);
							else if (sizeof(T) == 2)
								pixeldata = (T)D3D_ByteSwap16(pixeldata);
						}
						ubpixel[l] = QINDIEGL_CLAMP(((GLfloat)pixeldata / std::numeric_limits<T>::max()) * 255);
					}
				} else {
					T pixeldata = *ppixel;
					if (D3DState.ClientPixelStoreState.unpackSwapBytes) {
		...
		for ( int j = 0; j < height; j++ ) {
			for ( int k = 0; k < width; k++ ) {
				...
				for ( int l = 0; l < pixelsize; l++ ) {
					ubpixel[l] = QINDIEGL_CLAMP(ppixel[l] * 255);
				}
					...
					if (pack_mode == PP_TYPE_UNPACKED) {
						for ( int l = 0; l < fmtpixelsize; l++ ) {
							...
							if (D3DState.ClientPixelStoreState.packSwapBytes) {
								if (sizeof(T) == 4)
									ppixel[l] = (T)D3D_ByteSwap32(ppixel[l]);
								else if (sizeof(T) == 2)
									ppixel[l] = (T)D3D_ByteSwap16(ppixel[l]);
							}
						}
					} else {
						*ppixel = D3DPixels_AssemblePackedPixel<T>( pack_mode, fmtpixelsize, ubpixel );
						if (D3DState.ClientPixelStoreState.packSwapBytes) {
							if (sizeof(T) == 4)
								*ppixel = (T)D3D_ByteSwap32(*ppixel);
							else if (sizeof(T) == 2)
								*ppixel = (T)D3D_ByteSwap16(*ppixel);
						}
					}
				}
			}
...
HRESULT D3DPixels_GetPixelInfo( GLenum format, int &pixelSize, DWORD &channelMask, DWORD &flags  )
{
	...
	case GL_LUMINANCE:
	case GL_COLOR_INDEX:
		mask = 0x7;	//exclude alpha channel
		pixelSize = 1;
		break;
	case GL_INTENSITY:
		pixelSize = 1;
...
OPENGL_API void WINAPI glReadPixels(GLint x,  GLint y,  GLsizei width,  GLsizei height,  GLenum format,  GLenum type,  GLvoid *pixels)
{
	...
	/*	D3DSURFACE_DESC desc;
		...
		hr = D3DGlobal.pDevice->StretchRect( lpRenderTarget, NULL, lpDS, NULL, D3DTEXF_NONE );
		if (FAILED(hr)) {
			D3DGlobal.lastError = hr;
...
OPENGL_API void WINAPI glCopyPixels(GLint x,  GLint y,  GLsizei width,  GLsizei height,  GLenum type)
{
	logPrintf("WARNING: glCopyPixels is not supported\n");
}

OPENGL_API void glDrawPixels(GLsizei width,  GLsizei height,  GLenum format,  GLenum type,  const GLvoid *pixels)
{
...
OPENGL_API void WINAPI glGetPixelMapusv(GLenum map,  GLushort *values)
{
	...
	for (DWORD i = 0; i < mapSize; i++) {
		values[i] = (GLuint)(mapPointer[i] * USHRT_MAX);
	}
}
...
OPENGL_API void WINAPI glPixelMapuiv(GLenum map,  GLsizei mapsize,  const GLuint *values)
{
	...
	*mapSizePointer = mapsize;
	for (int i = 0; i < mapsize; i++) {
		mapPointer[i] = (GLfloat)(values[i] / UINT_MAX);
	}
}
...
OPENGL_API void WINAPI glPixelMapusv(GLenum map,  GLsizei mapsize,  const GLushort *values)
{
	...
	*mapSizePointer = mapsize;
	for (int i = 0; i < mapsize; i++) {
		mapPointer[i] = (GLfloat)(values[i] / USHRT_MAX);
	}
}
...
OPENGL_API void WINAPI glPixelZoom(GLfloat xfactor,  GLfloat yfactor)
{
	logPrintf("WARNING: glPixelZoom is not supported\n");
}

OPENGL_API void WINAPI glRasterPos2d (GLdouble x,  GLdouble y)
{
	logPrintf("WARNING: glRasterPos2d is not supported\n");
}
OPENGL_API void WINAPI glRasterPos2dv (const GLdouble *v)
{
	logPrintf("WARNING: glRasterPos2dv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos2f (GLfloat x,  GLfloat y)
{
	logPrintf("WARNING: glRasterPos2f is not supported\n");
}
OPENGL_API void WINAPI glRasterPos2fv (const GLfloat *v)
{
	logPrintf("WARNING: glRasterPos2fv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos2i (GLint x,  GLint y)
{
	logPrintf("WARNING: glRasterPos2i is not supported\n");
}
OPENGL_API void WINAPI glRasterPos2iv (const GLint *v)
{
	logPrintf("WARNING: glRasterPos2iv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos2s (GLshort x,  GLshort y)
{
	logPrintf("WARNING: glRasterPos2s is not supported\n");
}
OPENGL_API void WINAPI glRasterPos2sv (const GLshort *v)
{
	logPrintf("WARNING: glRasterPos2sv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos3d (GLdouble x,  GLdouble y,  GLdouble z)
{
	logPrintf("WARNING: glRasterPos3d is not supported\n");
}
OPENGL_API void WINAPI glRasterPos3dv (const GLdouble *v)
{
	logPrintf("WARNING: glRasterPos3dv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos3f (GLfloat x,  GLfloat y,  GLfloat z)
{
	logPrintf("WARNING: glRasterPos3f is not supported\n");
}
OPENGL_API void WINAPI glRasterPos3fv (const GLfloat *v)
{
	logPrintf("WARNING: glRasterPos3fv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos3i (GLint x,  GLint y,  GLint z)
{
	logPrintf("WARNING: glRasterPos3i is not supported\n");
}
OPENGL_API void WINAPI glRasterPos3iv (const GLint *v)
{
	logPrintf("WARNING: glRasterPos3iv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos3s (GLshort x,  GLshort y,  GLshort z)
{
	logPrintf("WARNING: glRasterPos3s is not supported\n");
}
OPENGL_API void WINAPI glRasterPos3sv (const GLshort *v)
{
	logPrintf("WARNING: glRasterPos3sv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos4d (GLdouble x,  GLdouble y,  GLdouble z,  GLdouble w)
{
	logPrintf("WARNING: glRasterPos4d is not supported\n");
}
OPENGL_API void WINAPI glRasterPos4dv (const GLdouble *v)
{
	logPrintf("WARNING: glRasterPos4dv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos4f (GLfloat x,  GLfloat y,  GLfloat z,  GLfloat w)
{
	logPrintf("WARNING: glRasterPos4f is not supported\n");
}
OPENGL_API void WINAPI glRasterPos4fv (const GLfloat *v)
{
	logPrintf("WARNING: glRasterPos4fv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos4i (GLint x,  GLint y,  GLint z,  GLint w)
{
	logPrintf("WARNING: glRasterPos4i is not supported\n");
}
OPENGL_API void WINAPI glRasterPos4iv (const GLint *v)
{
	logPrintf("WARNING: glRasterPos4iv is not supported\n");
}
OPENGL_API void WINAPI glRasterPos4s (GLshort x,  GLshort y,  GLshort z,  GLshort w)
{
	logPrintf("WARNING: glRasterPos4s is not supported\n");
}
OPENGL_API void WINAPI glRasterPos4sv (const GLshort *v)
{
	logPrintf("WARNING: glRasterPos4sv is not supported\n");
}
OPENGL_API void WINAPI glBitmap(GLsizei width,  GLsizei height,  GLfloat xorig,  GLfloat yorig,  GLfloat xmove,  GLfloat ymove,  const GLubyte *bitmap)
{
