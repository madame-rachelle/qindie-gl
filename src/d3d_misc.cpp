...
OPENGL_API void WINAPI glClear(GLbitfield mask)
{
	...
	HRESULT hr = D3DGlobal.pDevice->Clear( 0, NULL, clearMask & ~(D3DGlobal.ignoreClearMask), D3DState.ColorBufferState.clearColor, D3DState.DepthBufferState.clearDepth, D3DState.StencilBufferState.clearStencil );
...
OPENGL_API void WINAPI glClearIndex(GLfloat s)
{
	// d3d doesn't support color index mode
}
OPENGL_API void WINAPI glClearAccum(GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha)
{
...
OPENGL_API void WINAPI glIndexMask(GLuint mask)
{
	// d3d doesn't support color index mode
}
OPENGL_API void WINAPI glDrawBuffer(GLenum mode)
{
	// d3d doesn't like us messing with the front buffer, so here
	// we just silently ignore requests to change the draw buffer
}
OPENGL_API void WINAPI glReadBuffer(GLenum mode)
{
	// d3d doesn't like us messing with the front buffer, so here
	// we just silently ignore requests to change the draw buffer
}
...
OPENGL_API void WINAPI glPolygonStipple(const GLubyte *mask)
{
...
OPENGL_API void WINAPI glGetPolygonStipple(GLubyte *mask)
{
	static bool warningPrinted = false;
	if (!warningPrinted) {
		warningPrinted = true;
...
OPENGL_API void WINAPI glLineStipple(GLint factor,  GLushort pattern)
{
	static bool warningPrinted = false;
	if (!warningPrinted) {
		warningPrinted = true;
...
OPENGL_API void WINAPI glLineWidth(GLfloat width)
{
	static bool warningPrinted = false;
	if (!warningPrinted) {
		warningPrinted = true;
...
OPENGL_API void WINAPI glAccum(GLenum op,  GLfloat value)
{
	// d3d doesn't support accumulator
	logPrintf("WARNING: glAccum: accumulator is not supported\n");
}
OPENGL_API void WINAPI glFlush(void)
{
	// ignore
}
OPENGL_API void WINAPI glFinish(void)
{
...
OPENGL_API void WINAPI glDebugEntry(DWORD dwArg1, DWORD dwArg2)
{
