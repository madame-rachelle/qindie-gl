...
void D3DState_SetCullMode( const D3DState_t *state )
{
	if (!D3DGlobal.pDevice) return;
	HRESULT hr = S_OK;

	if (!state->EnableState.cullEnabled) {
		hr = D3DGlobal.pDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);
	} else {
		if (state->PolygonState.frontFace == GL_CCW)
			hr = D3DGlobal.pDevice->SetRenderState(D3DRS_CULLMODE, (state->PolygonState.cullMode == GL_BACK) ? D3DCULL_CW : D3DCULL_CCW );
		else
			hr = D3DGlobal.pDevice->SetRenderState(D3DRS_CULLMODE, (state->PolygonState.cullMode == GL_BACK) ? D3DCULL_CCW : D3DCULL_CW );
	}
	if (FAILED(hr)) D3DGlobal.lastError = hr;
}

void D3DState_SetDepthBias( const D3DState_t *state )
{
	if (!D3DGlobal.pDevice) return;
	HRESULT hr = S_OK;

	if (!state->EnableState.depthBiasEnabled) {
		...
		hr = D3DGlobal.pDevice->SetRenderState(D3DRS_SLOPESCALEDEPTHBIAS, UTIL_FloatToDword(state->PolygonState.depthBiasFactor));
		 if (SUCCEEDED(hr)) hr = D3DGlobal.pDevice->SetRenderState(D3DRS_DEPTHBIAS, UTIL_FloatToDword(state->PolygonState.depthBiasUnits));
	}
	
	if (FAILED(hr)) D3DGlobal.lastError = hr;
}

bool D3DState_SetMatrixMode( const D3DState_t *state )
{
	switch ( state->TransformState.matrixMode ) {
		case GL_MODELVIEW:
...
void D3DState_AssureBeginScene( void )
{
...
static void D3DState_SetTransform( void )
{
	HRESULT hr;
...
static void D3DState_SetLight( void )
{
...
static void D3DState_SetTextureEnvCombine( D3DState_t *state, int stage, int sampler )
{
	DWORD colorOp = UTIL_GLtoD3DTextureCombineOp( D3DState.TextureState.TextureCombineState[stage].colorOp, D3DState.TextureState.TextureCombineState[stage].colorScale );
	DWORD alphaOp = UTIL_GLtoD3DTextureCombineOp( D3DState.TextureState.TextureCombineState[stage].alphaOp, D3DState.TextureState.TextureCombineState[stage].alphaScale );

	DWORD colorArg1 = UTIL_GLtoD3DTextureCombineColorArg( D3DState.TextureState.TextureCombineState[stage].colorArg1, D3DState.TextureState.TextureCombineState[stage].colorOperand1 );
	DWORD colorArg2 = UTIL_GLtoD3DTextureCombineColorArg( D3DState.TextureState.TextureCombineState[stage].colorArg2, D3DState.TextureState.TextureCombineState[stage].colorOperand2 );
	DWORD alphaArg1 = UTIL_GLtoD3DTextureCombineAlphaArg( D3DState.TextureState.TextureCombineState[stage].alphaArg1, D3DState.TextureState.TextureCombineState[stage].alphaOperand1 );
	DWORD alphaArg2 = UTIL_GLtoD3DTextureCombineAlphaArg( D3DState.TextureState.TextureCombineState[stage].alphaArg2, D3DState.TextureState.TextureCombineState[stage].alphaOperand2 );
...
//	logPrintf("Stage %i, sampler %i: ALPHA op %d, arg1 %d, arg2 %d, scale %d\n", stage, sampler, alphaOp, alphaArg1, alphaArg2, D3DState.TextureState.TextureCombineState[stage].alphaScale);
	...
	if (colorOp == D3DTOP_LERP) {
		DWORD colorArg3 = UTIL_GLtoD3DTextureCombineColorArg( D3DState.TextureState.TextureCombineState[stage].colorArg3, D3DState.TextureState.TextureCombineState[stage].colorOperand3 );
	...
	if (alphaOp == D3DTOP_LERP) {
		DWORD alphaArg3 = UTIL_GLtoD3DTextureCombineAlphaArg( D3DState.TextureState.TextureCombineState[stage].alphaArg3, D3DState.TextureState.TextureCombineState[stage].alphaOperand3 );
		D3DGlobal.pDevice->SetTextureStageState( sampler, D3DTSS_ALPHAARG0, alphaArg3 );
	}
}

static void D3DState_SetTextureEnv( D3DState_t *state, int stage, int sampler, eTexTypeInternal intformat )
{
...
//		logPrintf("Stage %i, sampler %i: GL_COMBINE_ARB\n", stage, sampler);
	...
	if (D3DGlobal.hD3DCaps.RasterCaps & D3DPRASTERCAPS_MIPMAPLODBIAS)
		D3DGlobal.pDevice->SetSamplerState( sampler, D3DSAMP_MIPMAPLODBIAS, UTIL_FloatToDword(state->TextureState.textureLodBias[stage])  );
}

void D3DState_SetTexture( D3DState_t *state )
{
	if (!state->TextureState.textureSamplerStateChanged)
		return;
	...
	for (int i = 0; i < D3DGlobal.maxActiveTMU; i++) {
		...
		if (matrixChanged) {
			state->textureMatrixModified[i] = false;
		...
		D3DTextureObject *bestTexture = NULL;
		DWORD *bestTextureChanged = NULL;
		int currentTarget = 0;
		...
		DWORD textureStateChanged = state->TextureState.textureEnableChanged | state->TextureState.textureStateChanged[i];
		if (!textureStateChanged) {
			currentSampler++;
			continue;
		}

		state->TextureState.textureStateChanged[i] = FALSE;
		...
		hr = D3DGlobal.pDevice->SetTexture( currentSampler, bestTexture ? bestTexture->GetD3DTexture() : NULL );
		if (FAILED(hr)) {
			D3DGlobal.lastError = hr;
			break;
		}
		...
		if (bestTextureChanged) {
			DWORD dwBestTextureChanged = state->TextureState.textureEnableChanged | (*bestTextureChanged);
	...
	state->TextureState.textureEnableChanged = FALSE;
}

void D3DState_Check( void )
{
	...
	D3DState_SetLight();
	D3DState_SetTexture(&D3DState);
}

void D3DState_Apply( D3DState_t *state, GLbitfield mask )
{
	if (mask & GL_COLOR_BUFFER_BIT) {
		D3DGlobal.pDevice->SetRenderState(D3DRS_ALPHAFUNC, state->ColorBufferState.alphaTestFunc);
		D3DGlobal.pDevice->SetRenderState(D3DRS_ALPHAREF, state->ColorBufferState.alphaTestReference);
		D3DGlobal.pDevice->SetRenderState(D3DRS_SRCBLEND, state->ColorBufferState.alphaBlendSrcFunc);
		D3DGlobal.pDevice->SetRenderState(D3DRS_DESTBLEND, state->ColorBufferState.alphaBlendDstFunc);
		D3DGlobal.pDevice->SetRenderState(D3DRS_COLORWRITEENABLE, state->ColorBufferState.colorWriteMask);
		D3DGlobal.pDevice->SetRenderState(D3DRS_ALPHATESTENABLE, state->EnableState.alphaTestEnabled);
		D3DGlobal.pDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, state->EnableState.alphaBlendEnabled);
		...
		D3DGlobal.pDevice->SetRenderState( D3DRS_BLENDOP, state->ColorBufferState.alphaBlendOp );
	}
	if (mask & GL_DEPTH_BUFFER_BIT) {
		D3DGlobal.pDevice->SetRenderState(D3DRS_ZFUNC, state->DepthBufferState.depthTestFunc);
		D3DGlobal.pDevice->SetRenderState(D3DRS_ZWRITEENABLE, state->DepthBufferState.depthWriteMask);
		D3DGlobal.pDevice->SetRenderState(D3DRS_ZENABLE, state->EnableState.depthTestEnabled);
	}
	if (mask & GL_STENCIL_BUFFER_BIT) {
		D3DGlobal.pDevice->SetRenderState(D3DRS_STENCILFUNC, state->StencilBufferState.stencilTestFunc);
		D3DGlobal.pDevice->SetRenderState(D3DRS_STENCILREF, state->StencilBufferState.stencilTestReference);
		D3DGlobal.pDevice->SetRenderState(D3DRS_STENCILMASK, state->StencilBufferState.stencilTestMask);
		D3DGlobal.pDevice->SetRenderState(D3DRS_STENCILWRITEMASK, state->StencilBufferState.stencilWriteMask);
		D3DGlobal.pDevice->SetRenderState(D3DRS_STENCILFAIL, state->StencilBufferState.stencilTestFailFunc);
		D3DGlobal.pDevice->SetRenderState(D3DRS_STENCILZFAIL, state->StencilBufferState.stencilTestZFailFunc);
		D3DGlobal.pDevice->SetRenderState(D3DRS_STENCILPASS, state->StencilBufferState.stencilTestPassFunc);
		D3DGlobal.pDevice->SetRenderState(D3DRS_STENCILENABLE, state->EnableState.stencilTestEnabled);
		if (D3DGlobal.hD3DCaps.StencilCaps & D3DSTENCILCAPS_TWOSIDED) {
			D3DGlobal.pDevice->SetRenderState(D3DRS_CCW_STENCILFUNC, state->StencilBufferState.stencilTestFuncCCW);
			D3DGlobal.pDevice->SetRenderState(D3DRS_CCW_STENCILFAIL, state->StencilBufferState.stencilTestFailFuncCCW);
			D3DGlobal.pDevice->SetRenderState(D3DRS_CCW_STENCILZFAIL, state->StencilBufferState.stencilTestZFailFuncCCW);
			D3DGlobal.pDevice->SetRenderState(D3DRS_CCW_STENCILPASS, state->StencilBufferState.stencilTestPassFuncCCW);
			D3DGlobal.pDevice->SetRenderState(D3DRS_TWOSIDEDSTENCILMODE, state->EnableState.twoSideStencilEnabled );
		}
	}
	if (mask & GL_FOG_BIT) {
		D3DGlobal.pDevice->SetRenderState(D3DRS_FOGENABLE, state->EnableState.fogEnabled);
		D3DGlobal.pDevice->SetRenderState(D3DRS_FOGCOLOR, state->FogState.fogColor);
		D3DGlobal.pDevice->SetRenderState(D3DRS_FOGSTART, UTIL_FloatToDword(state->FogState.fogStart));
		D3DGlobal.pDevice->SetRenderState(D3DRS_FOGEND, UTIL_FloatToDword(state->FogState.fogEnd));
		D3DGlobal.pDevice->SetRenderState(D3DRS_FOGDENSITY, UTIL_FloatToDword(state->FogState.fogDensity));
		D3DGlobal.pDevice->SetRenderState(D3DRS_FOGTABLEMODE, (!D3DState.FogState.fogCoordMode && state->HintState.fogHint <= 1) ? state->FogState.fogMode : D3DFOG_NONE);
		D3DGlobal.pDevice->SetRenderState(D3DRS_FOGVERTEXMODE, (D3DState.FogState.fogCoordMode || state->HintState.fogHint <= 1) ? D3DFOG_NONE : state->FogState.fogMode);
	}
	if (mask & GL_HINT_BIT) {
		if (!(mask & GL_FOG_BIT)) {
			D3DGlobal.pDevice->SetRenderState(D3DRS_FOGTABLEMODE, (!D3DState.FogState.fogCoordMode && state->HintState.fogHint <= 1) ? state->FogState.fogMode : D3DFOG_NONE);
	...
	if (mask & GL_LIGHTING_BIT) {
		D3DGlobal.pDevice->SetRenderState(D3DRS_SHADEMODE, state->LightingState.shadeMode);
		D3DGlobal.pDevice->SetRenderState(D3DRS_LIGHTING, state->EnableState.lightingEnabled);
		D3DGlobal.pDevice->SetRenderState(D3DRS_LOCALVIEWER, state->LightingState.lightModelLocalViewer);
		D3DGlobal.pDevice->SetRenderState(D3DRS_AMBIENT, state->LightingState.lightModelAmbient);
		for (int i = 0; i < D3DGlobal.maxActiveLights; i++) {
			D3DGlobal.pDevice->LightEnable(i, state->EnableState.lightEnabled[i] );
			state->LightingState.lightModified[i] = TRUE;
		}
	}
	if (mask & GL_POINT_BIT) {
		D3DGlobal.pDevice->SetRenderState(D3DRS_POINTSIZE, UTIL_FloatToDword(state->PointState.pointSize));
	}
	if (mask & GL_POLYGON_BIT) {
		D3DGlobal.pDevice->SetRenderState(D3DRS_FILLMODE, state->PolygonState.fillMode);
		D3DState_SetCullMode( state );
		D3DState_SetDepthBias( state );
	}
	if (mask & GL_ENABLE_BIT) {
		D3DGlobal.pDevice->SetRenderState(D3DRS_ALPHATESTENABLE, state->EnableState.alphaTestEnabled);
		D3DGlobal.pDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, state->EnableState.alphaBlendEnabled);
		...
		D3DGlobal.pDevice->SetRenderState(D3DRS_FOGENABLE, state->EnableState.fogEnabled);
		D3DGlobal.pDevice->SetRenderState(D3DRS_LIGHTING, state->EnableState.lightingEnabled);
		D3DGlobal.pDevice->SetRenderState(D3DRS_DITHERENABLE, state->EnableState.ditherEnabled);
		D3DGlobal.pDevice->SetRenderState(D3DRS_NORMALIZENORMALS, state->EnableState.normalizeEnabled);
		if (!(mask & GL_POLYGON_BIT)) {
			D3DState_SetCullMode( state );
			D3DState_SetDepthBias( state );
		}
		if (!(mask & GL_TEXTURE_BIT)) {
			D3DState_SetTexture( state );
		}
		if (D3DGlobal.hD3DCaps.StencilCaps & D3DSTENCILCAPS_TWOSIDED) {
			D3DGlobal.pDevice->SetRenderState(D3DRS_TWOSIDEDSTENCILMODE, state->EnableState.twoSideStencilEnabled );
		}
	...
	if (mask & GL_TEXTURE_BIT) {
		D3DState_SetTexture( state );
		D3DGlobal.pDevice->SetRenderState( D3DRS_TEXTUREFACTOR, state->TextureState.textureEnvColor );
	}
	if (mask & GL_TRANSFORM_BIT) {
		D3DGlobal.pDevice->SetRenderState(D3DRS_NORMALIZENORMALS, state->EnableState.normalizeEnabled);
	}
...
void D3DState_SetDefaults( void )
{
	assert( D3DGlobal.pD3D != NULL );
	assert( D3DGlobal.pDevice != NULL );
	...
	D3DState.ColorBufferState.colorWriteMask = ~0;
	...
	D3DState.StencilBufferState.stencilTestMask = ~0;
	D3DState.StencilBufferState.stencilWriteMask = ~0;
	...
	D3DState_Apply( &D3DState, GL_ALL_ATTRIB_BITS );
...
OPENGL_API void WINAPI glPopAttrib( void )
{
	if (!D3DStateCopyMask) {
	...
	D3DState_Apply( &D3DState, D3DStateCopyMask );

	D3DStateCopyMask = 0;
}

OPENGL_API void WINAPI glPopClientAttrib( void )
{
...
static void D3DState_EnableDisableState( GLenum cap, DWORD value )
{
	...
	case GL_POLYGON_OFFSET_FILL:
		...
		D3DState_SetDepthBias( &D3DState );
		break;
	case GL_SCISSOR_TEST:
