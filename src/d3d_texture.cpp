...
void D3DTextureObject :: FreeD3DTexture( void )
{
	if (m_pD3DTexture) {
...
HRESULT D3DTextureObject :: GetTexImage( GLint cubeface, GLint level, GLenum format,  GLenum type,  GLvoid *pixels )
{
	...
	if (m_target == GL_TEXTURE_3D_EXT) {
		D3DLOCKED_BOX lockrect;
		hr = m_pD3DVolumeTexture->LockBox( level, &lockrect, NULL, D3DLOCK_READONLY );
		if (FAILED(hr)) return hr;
		...
		D3DLOCKED_RECT lockrect;
		hr = m_pD3DCubeTexture->LockRect( (D3DCUBEMAP_FACES)cubeface, level, &lockrect, NULL, D3DLOCK_READONLY );
		if (FAILED(hr)) return hr;
		...
		D3DLOCKED_RECT lockrect;
		hr = m_pD3DTexture->LockRect( level, &lockrect, NULL, D3DLOCK_READONLY );
		if (FAILED(hr)) return hr;
...
HRESULT D3DTextureObject :: DumpTexture( void )
{
	HRESULT hr;
	GLubyte *srcdata;
	GLint pitch;
	...
	dumpBuffer[2] = 2;
	dumpBuffer[12] = m_width & 255;
	dumpBuffer[13] = m_width >> 8;
	dumpBuffer[14] = m_height & 255;
	dumpBuffer[15] = m_height >> 8;
	dumpBuffer[16] = 32;
	...
	if (m_target == GL_TEXTURE_3D_EXT) {
		...
		D3DLOCKED_RECT lockrect;
		hr = m_pD3DTexture->LockRect( 0, &lockrect, NULL, D3DLOCK_READONLY );
		if (FAILED(hr)) return hr;
...
void D3DTextureObject :: CheckMipmapAutogen( void )
{
	if (m_autogenMipmaps)
...
static void D3DTex_LoadSubImage(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLenum type, const GLvoid *pixels)
{
	...
	int currentTMU = D3DState.TextureState.currentTMU;
	assert(D3DState.TextureState.currentTexture[currentTMU][targetIndex] != NULL);
	assert( D3DState.TextureState.currentTexture[currentTMU][targetIndex]->GetD3DTexture() != NULL );
...
{
	assert( D3DGlobal.pObjectBuffer != NULL );
...
OPENGL_API void WINAPI glBindTexture(GLenum target, GLuint texture)
{
	...
	if ((pTexture->GetD3DTexture() != NULL) && (pTexture->GetTarget() != target)) {
		D3DState.TextureState.currentTexture[currentTMU][targetIndex] = D3DGlobal.defaultTexture[targetIndex];
		D3DGlobal.lastError = E_INVALIDARG;
		return;
	}
...
{
	D3DTextureObject *currentTexture = NULL;
...
OPENGL_API void WINAPI glActiveTexture(GLenum texture)
{
	...
	if (D3DState.TransformState.matrixMode == GL_TEXTURE)
		D3DState_SetMatrixMode( &D3DState );
}
